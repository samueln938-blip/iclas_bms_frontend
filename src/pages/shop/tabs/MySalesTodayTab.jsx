...
// ✅ Money input that feels natural (easy delete/backspace) + comma formatting
function MoneyInput({ value, onChange, placeholder, style, disabled, title }) {
  ...
  const countDigitsBeforeCursor = (s, cursorIdx) => {
    const left = s.slice(0, Math.max(0, cursorIdx));
    return (left.match(/\d/g) || []).length;
  };

  const indexForDigitCount = (formatted, digitCount) => {
  ...
  return (
    <input
      ...
      onChange={(e) => {
        ...
        // restore cursor after reformat
        window.requestAnimationFrame(() => {
          const el = ref.current;
          if (!el) return;
          const nextDisplay = formatWithCommas(nextDigits);
          const pos = indexForDigitCount(nextDisplay, digitsBefore);
          try {
  ...
  const updatePad = (field, rawValue) => {
    setPad((prev) => {
      if (field === "itemId") {
        ...
        const autoPrice =
          row?.selling_price_per_piece != null ? digitsOnly(String(row.selling_price_per_piece)) : "";
      ...
      if (field === "agreedPricePerPiece") return { ...prev, agreedPricePerPiece: digitsOnly(rawValue) };
      return prev;
    });
  };
  ...
  const handleAddItemToSale = () => {
    ...
    setSaleLines((prev) => [
      ...prev,
      {
        ...
        unitPrice: agreedPricePerPiece, // store as number (we will accept string while editing)
        total,
        profit: totalProfit,
      },
    ]);
  ...
  const loadSaleForEdit = async (saleId, focusServerLineId = null) => {
    ...
    try {
      ...
      const collectedNowForCredit =
        isCredit && sale.amount_collected_now != null ? digitsOnly(String(sale.amount_collected_now)) : "0";
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* ✅ agreed price (MoneyInput) */}
          <div style={{ position: "relative", width: "100%" }}>
            <MoneyInput
              value={pad.agreedPricePerPiece}
              onChange={(d) => updatePad("agreedPricePerPiece", d)}
            ...
            <button
              ...
              onClick={() =>
                openCalculator(pad.agreedPricePerPiece, (amt) =>
                  updatePad("agreedPricePerPiece", digitsOnly(String(amt)))
                )
              }
                  ...
                  <td style={{ padding: "8px 4px", textAlign: "right" }}>
                    {isEditingLine ? (
                      <MoneyInput
                        ...
                        onChange={(d) => updateSaleLine(line.id, { unitPrice: d })}
                      ...
                      formatMoney(num(line.unitPrice))
                    )}
                  </td>
      ...
      {/* CUSTOMER + PAYMENT (UNCHANGED except MoneyInput for collected-now) */}
              ...
              <div style={{ padding: "10px 10px", borderRadius: "14px", backgroundColor: "#f9fafb", border: "1px solid #e5e7eb", marginBottom: "10px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 44px", gap: "10px", alignItems: "center" }}>
                  <MoneyInput
                    ...
                    onChange={(d) => setAmountCollectedNow(d)}
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      openCalculator(amountCollectedNow, (amt) => setAmountCollectedNow(digitsOnly(String(amt))))
                    }
                    title="Calculator"
